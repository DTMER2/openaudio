...
public final class Bus: @unchecked Sendable {
    ...
    public static func attach(index: Int, captureRate: Double,
        ...
        if output == .single16ch {
            // Guard the channel count of stream 0 specifically — the consumer
            // IOProc writes only into buffer 0, so a multi-stream device whose
            // TOTAL count is 16 (e.g. 8 stereo streams) would pass a total-count
            // check yet leave pairs >= 2 silent.
            let outCh = DeviceUtil.streamChannelLayout(vdev, scope: kAudioObjectPropertyScopeOutput).first ?? 0
            guard outCh >= channelOffset + 2 else {
                throw OAError("Device '\(uid)' has \(outCh) channels in its first output stream; " +
                              "bus \(index + 1) needs channels \(channelOffset + 1)/\(channelOffset + 2).")
            }
        }

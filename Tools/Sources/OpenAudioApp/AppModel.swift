...
final class AppModel {
    ...
    /// pairs of the single "OpenAudio 16ch" device (DAW-friendly).
    ...
    /// pair-packing on device 1. Restarts a running engine; the driver's
    /// device list is resized on the correct side of the restart so no live
    /// consumer IOProc ever sits on a device being removed.
    func setOutputMode(_ mode: BusOutputMode) {
        ...
        let finish: () -> Void = { [weak self] in
            DispatchQueue.main.async { self?.busOpInProgress = false }
        }
        if mode == .separateDevices {
            // Grow the device list first; the rebuilt engine then attaches
            // one device per bus.
            engineOps.async { [weak self] in
                do { _ = try OpenAudioControlPlane.setDeviceCount(count) }
                catch { DispatchQueue.main.async { self?.report(error, permission: false) } }
            }
            if isRunning { rebuildEngine(recording: false) }
            engineOps.async { finish() }
        } else {
    ...
    private func rebuildEngine(recording: Bool) {
        ...
        engineOps.async { [weak self] in
            ...
            do {
                ...
                self.opsEngine = e
                self.setOpsLaneMapping(tapPIDs: snap.tapPIDs, systemMode: snap.systemMode,
